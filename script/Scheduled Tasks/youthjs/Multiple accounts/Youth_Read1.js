                ...
                if (readres.error_code == '0' && data.indexOf("read_score") > -1 && readres.items.read_score > 0) {
                    ...
                    if ($.index % 2 == 0) {
                        if ($.isNode() && process.env.YOUTH_ATIME) {
                            ...
                            timebodyVal = $.getdata('autotime_zq1');
                        }
                        await readTime()
                    };
...
function removebody() {
  ...
  if (articlebody !== ReadArr[0]) {
      smallbody = $.getdata('youth_autoread1').replace("&" + articlebody, "");
  } else {
      smallbody = $.getdata('youth_autoread1').replace(articlebody + "&", "")
  }
  $.setdata(smallbody, 'youth_autoread1')
}
