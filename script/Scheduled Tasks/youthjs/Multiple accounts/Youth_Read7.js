...
const $ = new Env("中青看点阅读7")
//const notify = $.isNode() ? require('./sendNotify') : '';
let ReadArr = [], timebodyVal ="";
let YouthBody = $.getdata('youth_autoread7')||$.getdata("zqgetbody_body7");
let smallzq = $.getdata('youth_cut7');
let indexLast = $.getdata('zqbody_index7');
...
let lastIndex = $.getdata('zqbody_index7')
                ...
                if (readres.error_code == '0' && data.indexOf("read_score") > -1 && readres.items.read_score > 0) {
                    ...
                    if ($.index % 2 == 0) {
                        if ($.isNode() && process.env.YOUTH_ATIME) {
                            ...
                            timebodyVal = $.getdata('autotime_zq7');
                        }
                        await readTime()
                    };
...
function removebody() {
  ...
  if (articlebody !== ReadArr[0]) {
      smallbody = $.getdata('youth_autoread7').replace("&" + articlebody, "");
  } else {
      smallbody = $.getdata('youth_autoread7').replace(articlebody + "&", "")
  }
  $.setdata(smallbody, 'youth_autoread7')
}
