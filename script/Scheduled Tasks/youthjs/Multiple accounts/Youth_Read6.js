...
const $ = new Env("中青看点阅读6")
//const notify = $.isNode() ? require('./sendNotify') : '';
let ReadArr = [], timebodyVal ="";
let YouthBody = $.getdata('youth_autoread6')||$.getdata("zqgetbody_body6");
let smallzq = $.getdata('youth_cut6');
let indexLast = $.getdata('zqbody_index6');
...
let lastIndex = $.getdata('zqbody_index6')
                ...
                if (readres.error_code == '0' && data.indexOf("read_score") > -1 && readres.items.read_score > 0) {
                    ...
                    if ($.index % 2 == 0) {
                        if ($.isNode() && process.env.YOUTH_ATIME) {
                            ...
                            timebodyVal = $.getdata('autotime_zq6');
                        }
                        await readTime()
                    };
...
function removebody() {
  ...
  if (articlebody !== ReadArr[0]) {
      smallbody = $.getdata('youth_autoread6').replace("&" + articlebody, "");
  } else {
      smallbody = $.getdata('youth_autoread6').replace(articlebody + "&", "")
  }
  $.setdata(smallbody, 'youth_autoread6')
}
